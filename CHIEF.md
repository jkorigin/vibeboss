...
1. **No partner-specific runtime references in source.** No mentions of "Banana", "whatsapp-pa", "60163241921@c.us", partner's email, or any installation-specific paths in framework source. If a runtime value would help, use a placeholder like `{{LEAD_NAME}}` or `~/.vibeboss-workspace/...`. Templates substitute at install time.
...
- Anything in other ventures (`cortex/`, `wmc/`, `recrave/`, etc.) — same boundary as Boss
