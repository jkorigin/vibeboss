...
- Per-project memory has no clean home — runlogs and decisions about `whatsapp-pa` live in `vibeboss/office/`, conflating project memory with framework patterns.
...
~/ventures/vibeboss-workspace/             ← Runtime. Partner-owned. Never committed to OSS.
  ...
  └── projects/                             ← Parent of all partner-owned project codebases.
      └── whatsapp-pa/                      ← (moved from ~/ventures/vibeboss-workspace/projects/whatsapp-pa/)
```
...
1. `mkdir -p ~/ventures/vibeboss-workspace/{hq/{runlog,decisions,inbox,skills,follow-ups,secrets,projects/whatsapp-pa/decisions},labs/research,projects}`
2. Move project code:
   `mv ~/ventures/vibeboss-workspace/projects/whatsapp-pa  ~/ventures/vibeboss-workspace/projects/whatsapp-pa`
   `rmdir ~/ventures/vibeboss-projects` (now empty)
3. Move HQ runtime:
...
7. Sweep all moved files for absolute path references and update them. Specifically:
   - All `~/ventures/vibeboss-workspace/projects/whatsapp-pa` → `~/ventures/vibeboss-workspace/projects/whatsapp-pa`
...
8. Bootstrap per-project subdir for whatsapp-pa:
   - Create `hq/projects/whatsapp-pa/STATE.md` with current WA-PA state copied from cross-cutting STATE.md (anything WA-specific moves here; truly cross-project stays in `hq/STATE.md`).
   - Create stub `hq/projects/whatsapp-pa/lessons.md`, `notes.md`.
...
- `find ~/ventures/vibeboss -type f | xargs grep -l "vibeboss-projects" 2>/dev/null` returns nothing (or only inside this design doc, which references the old path historically).
...
- `bun start` from `~/ventures/vibeboss-workspace/projects/whatsapp-pa/` still boots the daemon, scans QR, reaches `connected` state (whatsapp-pa code's relative paths must still resolve after the move).
...
| The per-project memory routing rule sounds clean but in practice messages are ambiguous ("did this lesson belong to whatsapp-pa or HQ-cross-cutting?"). | Default behavior: I ask when unclear. LESSON-003 already says research-first on ambiguity; this is its application to memory routing. |
