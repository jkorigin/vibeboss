...
**Architecture:** Pure filesystem migration — no code generation. Move `office/` and `research/` and `crew.yml` and `vibeboss-workspace/projects/whatsapp-pa/` to new homes. Rewrite the two CLAUDE.md files (framework-reference vs boot-brief). Sweep all internal absolute-path references. Restart the running WhatsApp PA daemon from its new home and verify it still works end-to-end.
...
- `vibeboss-projects/` currently contains only `whatsapp-pa/` and will be removed entirely
...
cp -R ~/ventures/vibeboss-projects /tmp/vibeboss-snapshot-2026-05-26/vibeboss-projects
...
pkill -f "whatsapp-pa/data" 2>/dev/null
sleep 2
...
echo "--- vibeboss-projects/ contents ---" >> /tmp/vibeboss-migration-snapshot.txt
ls ~/ventures/vibeboss-projects/ >> /tmp/vibeboss-migration-snapshot.txt
...
**Files:**
- Create: `~/ventures/vibeboss-workspace/hq/{runlog,decisions,inbox,skills,follow-ups,secrets,projects/whatsapp-pa/decisions}`
...
mkdir -p \
  ...
  ~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa/decisions \
...
~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa
~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa/decisions
~/ventures/vibeboss-workspace/hq/runlog
~/ventures/vibeboss-workspace/hq/secrets
~/ventures/vibeboss-workspace/hq/skills
...
**Files:**
- Move: `~/ventures/vibeboss-workspace/projects/whatsapp-pa` → `~/ventures/vibeboss-workspace/projects/whatsapp-pa`
- Remove: `~/ventures/vibeboss-projects/` (parent, now empty)

- [ ] **Step 2.1: Move the project directory wholesale**

Run:
```bash
mv ~/ventures/vibeboss-workspace/projects/whatsapp-pa ~/ventures/vibeboss-workspace/projects/whatsapp-pa
ls ~/ventures/vibeboss-workspace/projects/
echo "---"
ls ~/ventures/vibeboss-workspace/projects/whatsapp-pa/ | head -15
```
Expected: `whatsapp-pa` appears under `projects/`; its contents include `package.json`, `src/`, `public/`, `node_modules/`, `data/`, etc.

- [ ] **Step 2.2: Remove the now-empty `vibeboss-projects/` parent**

Run:
```bash
ls ~/ventures/vibeboss-projects/ 2>&1
rmdir ~/ventures/vibeboss-projects/
ls ~/ventures/vibeboss-projects/ 2>&1
```
...
| `~/ventures/vibeboss-workspace/projects/whatsapp-pa/` | Partner's personal WhatsApp PA. |
...
- **Never write inside cortex** (`~/ventures/cortex/`). Mail-drop only.
...
Inspired by patterns developed in the realm of `~/ventures/cortex/`-style multi-venture offices, packaged for external use.
...
Apache License 2.0. Copyright Jin Kun Yong. `NOTICE` preserves attribution through forks. Commercial layers (paid dashboard, hosted service) are a Phase 2+ decision and not pre-committed.
...
Run:
```bash
echo "=== references to vibeboss-projects ==="
grep -rln "vibeboss-projects" \
...
grep -rln "vibeboss-workspace/labs/research" \
  ...
  2>/dev/null | grep -v node_modules | grep -v "/data/" | head -50
```
Expected: a list of files (mostly under `hq/runlog/`, `hq/decisions/`, the new spec/plan files, and possibly some inside `projects/whatsapp-pa/`).
...
Run:
```bash
# Most-specific first: vibeboss-workspace/projects/whatsapp-pa → vibeboss-workspace/projects/whatsapp-pa
find ~/ventures/vibeboss ~/ventures/vibeboss-workspace \
  ...
  -exec sed -i '' 's|vibeboss-workspace/projects/whatsapp-pa|vibeboss-workspace/projects/whatsapp-pa|g' {} +
...
## Task 8: Bootstrap per-project memory subdir for whatsapp-pa

**Files:**
- Create: `~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa/STATE.md`
- Create: `~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa/notes.md`
- Create: `~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa/lessons.md`

- [ ] **Step 8.1: Write the WhatsApp-PA project-specific STATE.md**

Write to `~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa/STATE.md`:

```markdown
# whatsapp-pa — project STATE
...
- Daemon location: `~/ventures/vibeboss-workspace/projects/whatsapp-pa/`
...
- Allowlist: `["238735841062923@lid"]` (ReCrave)
...
- 2026-05-25 — Workspace reorg to vibeboss-projects/ (superseded by topology migration)
...
Run:
```bash
ls -la ~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa/STATE.md
```
Expected: file exists.

- [ ] **Step 8.2: Write stub notes.md and lessons.md for the project**

Write to `~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa/notes.md`:

```markdown
# whatsapp-pa — running notes
...
Write to `~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa/lessons.md`:

```markdown
# whatsapp-pa — project LESSONS
...
Run:
```bash
ls -la ~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa/
```
...
grep -n "vibeboss-projects/" ~/ventures/vibeboss-workspace/hq/STATE.md
```
Expected: header reflects current phase; no leftover `office/` or `vibeboss-projects/` references (or if a few remain, they're historical mentions in context — review each).
...
3. `whatsapp-pa/` moved from `~/ventures/vibeboss-projects/` into `~/ventures/vibeboss-workspace/projects/`. Old parent `vibeboss-projects/` removed.
...
9. Per-project memory subdir bootstrapped for whatsapp-pa with project STATE/notes/lessons.
...
```
mkdir -p ~/ventures/vibeboss-workspace/{hq/{runlog,decisions,inbox,skills,follow-ups,secrets,projects/whatsapp-pa/decisions},labs/research,projects}
mv ~/ventures/vibeboss-workspace/projects/whatsapp-pa  ~/ventures/vibeboss-workspace/projects/whatsapp-pa
rmdir ~/ventures/vibeboss-projects
mv ~/ventures/vibeboss-workspace/hq/STATE.md  ~/ventures/vibeboss-workspace/hq/STATE.md
mv ~/ventures/vibeboss-workspace/hq/runlog/*.md  ~/ventures/vibeboss-workspace/hq/runlog/
...
- Created: ~/ventures/vibeboss-workspace/hq/projects/whatsapp-pa/{STATE.md, notes.md, lessons.md}
...
- Moved: ~80 files from vibeboss/office/, vibeboss-workspace/labs/research/, vibeboss-workspace/projects/whatsapp-pa/ to new locations
- Removed: vibeboss/office/, vibeboss-workspace/labs/research/, vibeboss-projects/
...
- Runtime tree (`~/ventures/vibeboss-workspace/`) holds: hq/ (16 dirs + boot brief + STATE + lessons + master crew.yml), labs/ (with research/), projects/ (with whatsapp-pa/).
...
**Files:**
- Run: `~/ventures/vibeboss-workspace/projects/whatsapp-pa/src/index.js` (via bun)
...
Run:
```bash
cd ~/ventures/vibeboss-workspace/projects/whatsapp-pa && nohup bun start > /tmp/wapa-direct.log 2>&1 &
...
- Check that `data/auth/` survived the move (`ls ~/ventures/vibeboss-workspace/projects/whatsapp-pa/data/`)
...
Have partner send a test message from another number (ReCrave, the allowlisted chat). Watch `tail -f /tmp/wapa-direct.log` for `[SEND] ReCrave «...» → «...» (Nms, $N)` line. If it lands within ~30s, the migration is fully validated.
...
```bash
echo "GATE 1: vibeboss-projects references remaining?"
find ~/ventures/vibeboss ~/ventures/vibeboss-workspace -type f \
  -not -path "*/node_modules/*" -not -path "*/data/*" \
  | xargs grep -l "vibeboss-projects" 2>/dev/null
...
- [ ] Daemon at `~/ventures/vibeboss-workspace/projects/whatsapp-pa/` is connected to WhatsApp and dashboard responds at port 3000.
