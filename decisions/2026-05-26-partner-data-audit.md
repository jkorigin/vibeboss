...
| `docs/superpowers/plans/2026-05-26-topology-hq-split.md` (×3) | Phone number `<phone>@c.us` (partner's WhatsApp identity) | Redacted to `<whatsapp-id>@c.us` / `<name>` |
...
| `docs/superpowers/specs/2026-05-26-crew-system-design.md` | `Banana`, `<example-project>` as concrete design examples | Specs documenting the crew system design process may use the real examples that validated the design. `<example-project>` is acceptable in docs/specs per framework canon (it's "the example app that validated the spawning model"). `Banana` is not in the restricted list. |
| `docs/superpowers/specs/2026-05-26-topology-hq-split-design.md` | `<example-project>` as example project | Same rationale — spec documenting design. |
| `docs/superpowers/plans/2026-05-26-topology-hq-split.md` | `<example-project>`, runtime paths (post-redaction) | Historical execution plan. Phone number redacted; remaining content describes process, not private identity. |
| `docs/superpowers/plans/2026-05-26-crew-system.md` | `<example-project>/inbox/` dirs | Execution plan using the example project. |
| `docs/superpowers/plans/2026-05-26-master-dashboard.md` | `<example-project>` in test assertion example | Acceptable as example. |
