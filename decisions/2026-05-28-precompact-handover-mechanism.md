...
The v0.2.4 rolling handover mechanism (commit `f5ec386`) shipped a `Stop` hook that fired every turn and rewrote `hq/handovers/_current.md` with the latest exchange + grepped markers. The design was intended to ensure that at the moment Claude Code's auto-compact fires, a fresh handover always exists for `compact-boot.sh` to inject.
