...
Source of the leak: same cargo-cult pattern as the `--max-budget-usd 10` issue fixed in commit 28dab5a — canon was drafted with a developer-fluent operator implicitly in mind, even though the target-user block says the opposite.
...
- v0.2.5 ships the architectural shift that fixes the second of the two weird traits partner flagged (the first — Boss quoting "$10 per spawn" — was fixed in commit 28dab5a).
